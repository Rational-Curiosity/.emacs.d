...
(defun unspecified-background-in-terminal (&optional frame)
  (let ((frame (or frame (selected-frame))))
    (unless (display-graphic-p frame)
    (set-face-background 'default "unspecified-bg" frame))))
(add-hook 'window-setup-hook 'unspecified-background-in-terminal)
(add-hook 'after-make-frame-functions 'unspecified-background-in-terminal)
